...
function defaultSettings() {
  return {
    ...
    shopBaseUrl: 'https://www.mymenes.com',
...
function shopBaseUrl(settings) {
  return String(settings?.shopBaseUrl || 'https://www.mymenes.com').replace(/\/$/, '');
}

/** Personal shop link. Always /r/{slug} so Netlify never 404s a pretty URL. */
function ambassadorShopLink(settings, slug) {
  const base = shopBaseUrl(settings);
  const s = slugify(slug);
  return s ? `${base}/r/${s}` : base;
}

function ambassadorPublicTools(settings, amb) {
  const s = settings || {};
  const app = String(s.ambassadorAppUrl || 'https://menesambassador.netlify.app').replace(/\/$/, '');
  return {
    link: ambassadorShopLink(s, amb?.slug),
    slug: amb?.slug || '',
    promoCode: amb?.promoCode || '',
    inviteLink: `${app}/join?ref=${encodeURIComponent(amb?.slug || '')}`,
  };
}

function findAmbassadorByLinkToken(program, token) {
  const raw = String(token || '').trim();
  if (!raw) return null;
  const slug = slugify(raw);
  const code = raw.toUpperCase().replace(/[^A-Z0-9]/g, '');
  const allowed = new Set(['active', 'pending']);
  const list = (program?.ambassadors || []).filter((a) => a && allowed.has(a.status));
  return (
    list.find((a) => a.slug === slug)
    || list.find((a) => String(a.promoCode || '').toUpperCase().replace(/[^A-Z0-9]/g, '') === code)
    || list.find((a) => slugify(a.promoCode || '') === slug)
    || null
  );
}
