const DEFAULT_PLATFORM = {
  ...
  sites: [{
    ...
    domains: ['boutiquemenes.netlify.app', 'mymenes.com', 'www.mymenes.com'],
    netlifyUrl: 'https://www.mymenes.com',
    status: 'active',
    plan: 'pro',
...
const DEFAULT_STORE = {
  site: {
    ...
    heroTitle: 'STREET LUXE. C\'EST MENES.',
    heroSubtitle: 'Vêtements et accessoires pour ceux qui créent la mode. Pas ceux qui la suivent.',
    currency: 'CAD',
    sections: { announcement: true, trustBar: true, why: true, design: true, gallery: true, reviews: true, bundle: false, emailCapture: true, faq: true, guarantee: false },
  },
  products: [],
  orders: [],
  customers: [],
...
async function writeSiteStore(siteId, data) {
  const store = await getBlobStore();
  const key = storeKey(siteId);
  try {
    const existing = await store.get(key, { type: 'json' });
    const existingCount = Array.isArray(existing?.products) ? existing.products.length : 0;
    const incomingCount = Array.isArray(data?.products) ? data.products.length : 0;
    if (existingCount > 0 && incomingCount === 0) {
      throw new Error(`Refus d'écraser le catalogue ${siteId} (${existingCount} produits) par une sauvegarde vide`);
    }
    if (existing && existingCount > 0) {
      await store.setJSON(`${key}:backup`, existing);
      await store.setJSON(`${key}:safe`, existing);
      if (siteId === 'menes') {
        await store.setJSON('store:backup', existing);
        await store.setJSON('store:safe', existing);
      }
    }
  } catch (e) {
    if (/Refus d'écraser/.test(e.message)) throw e;
  }
...
function knownSiteIds(platform) {
  const ids = new Set(['menes']);
  if (platform?.defaultSiteId) ids.add(platform.defaultSiteId);
  for (const site of platform?.sites || []) {
    if (site?.id) ids.add(String(site.id));
  }
  return ids;
}
...
async function resolveSiteId(host, querySiteId) {
  ...
  const requested = String(querySiteId || '').trim();
  if (requested && /^[a-z0-9_-]{1,40}$/i.test(requested) && knownSiteIds(platform).has(requested)) {
    return requested;
  }
