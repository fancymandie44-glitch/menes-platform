...
function tokenSecret() {
  const secret = process.env.AMBASSADOR_TOKEN_SECRET || process.env.ADMIN_PASSWORD || '';
  if (secret) return secret;
  if (process.env.NETLIFY || process.env.CONTEXT) {
    throw new Error('AMBASSADOR_TOKEN_SECRET or ADMIN_PASSWORD required');
  }
  return 'menes-ambassador-dev-secret';
}
...
function requireAdmin(event) {
  const auth = require('./admin-auth').checkAdminAuth(event);
