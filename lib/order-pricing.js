 ...
 */
const { computeShipping } = require('./admin-ops');
...
function buildTrustedOrder(store, raw) {
  ...
  for (const line of rawItems.slice(0, 40)) {
    ...
    if (opts.length) {
      ...
      size = size || '—';
      const left = findVariantStock(product, {});
      if (Number.isFinite(left) && left <= 0) {
        return { error: `Rupture de stock: ${product.name}` };
      }
      if (Number.isFinite(left) && qty > left) {
        return { error: `Stock insuffisant pour ${product.name} (max ${left})` };
      }
    }
    items.push({
      id: product.id,
  ...
  const shipping = computeShipping(store, customer, subtotal);
...
function applyQtyToProduct(product, variantMap, qty) {
  if (!product || !qty || !Number.isFinite(qty)) return;
  const list = Array.isArray(product.variants) ? product.variants : [];
  if (list.length) {
    const key = variantKeyFromMap(variantMap);
    const hit = list.find((v) => (v.key || variantKeyFromMap(v.options || {})) === key)
    ...
    product.stock = list.reduce((sum, v) => sum + Math.max(0, Number(v.stock) || 0), 0);
    return;
  }
  const s = Number(product.stock);
  if (Number.isFinite(s)) {
    product.stock = Math.max(0, s - qty);
  }
}

function decrementStockForOrder(store, order) {
  if (!store || !order || order.stockDecremented) return false;
  ...
  for (const line of Array.isArray(order.items) ? order.items : []) {
    ...
    const qty = Math.max(1, Math.min(20, parseInt(line.qty, 10) || 1));
    const variantMap = parseLineVariant(line, product);
    applyQtyToProduct(product, variantMap, qty);
  }
  order.stockDecremented = true;
  return true;
}

function restockStockForOrder(store, order) {
  if (!store || !order || !order.stockDecremented || order.stockRestocked) return false;
  const products = Array.isArray(store.products) ? store.products : [];
  for (const line of Array.isArray(order.items) ? order.items : []) {
    const product = products.find((p) => p.id === line.id || p.id === line.productId);
    if (!product) continue;
