...
function buildCustomerReceiptHtml(order, method, passport) {
  const passportHtml = passport?.code
    ? `<p style="color:#c9a84c;font-size:14px;letter-spacing:0.12em;text-transform:uppercase;margin:18px 0 8px">Passeport MENES</p>
       <p style="color:#f4f2ee;font-size:15px;margin:0 0 16px">Ton passeport <strong>${escHtml(passport.code)}</strong> est créé. Tes commandes restent liées à ${escHtml(order.customer?.email || '')}.</p>`
    : '';
  return brandShell({
    title: 'Confirmation de commande',
    bodyHtml: `<p style="color:#9a958c;font-size:15px;line-height:1.6;margin:0 0 16px">Merci pour votre confiance. Voici le récapitulatif de votre commande MENES.</p>${buildOrderTable(order, method, 'Confirmée')}${passportHtml}`,
    footerNote: 'Une question ? Répondez à cet email ou contactez-nous via la boutique.',
  });
}

function buildPaymentReminderHtml(order, shopUrl) {
  const url = String(shopUrl || 'https://www.mymenes.com').replace(/\/$/, '');
  return brandShell({
    title: 'Ta commande t’attend',
    bodyHtml: `<p style="color:#9a958c;font-size:15px;line-height:1.6;margin:0 0 16px">Tu as commencé une commande MENES (#${escHtml(order.id)}) et le paiement n’est pas terminé. Tes pièces sont encore disponibles — finalise ici :</p>
      <p style="margin:0 0 18px"><a href="${escHtml(url)}/#products" style="display:inline-block;background:#c9a84c;color:#050505;text-decoration:none;padding:12px 18px;font-weight:700">Reprendre ma commande</a></p>
      ${buildOrderTable(order, order.paymentMethod || 'en attente', 'Paiement incomplet')}`,
    footerNote: 'Si tu as déjà payé, ignore ce message.',
  });
}

function buildReviewAskHtml(order, shopUrl) {
  const url = String(shopUrl || 'https://www.mymenes.com').replace(/\/$/, '');
  const productId = (order.items && order.items[0] && (order.items[0].id || order.items[0].productId)) || '';
  const href = `${url}/?review=1${productId ? `&product=${encodeURIComponent(productId)}` : ''}`;
  return brandShell({
    title: 'Ton avis MENES',
    bodyHtml: `<p style="color:#9a958c;font-size:15px;line-height:1.6;margin:0 0 16px">Ta commande #${escHtml(order.id)} est en route (ou déjà chez toi). Un avis honnête aide les prochaines pièces — et la communauté MENES.</p>
      <p style="margin:0 0 18px"><a href="${escHtml(href)}" style="display:inline-block;background:#c9a84c;color:#050505;text-decoration:none;padding:12px 18px;font-weight:700">Laisser un avis</a></p>`,
    footerNote: 'Un clic, une minute. Merci.',
  });
}
...
async function notifyCustomer(order, method, passport) {
  ...
  const html = buildCustomerReceiptHtml(order, method, passport);
  try {
